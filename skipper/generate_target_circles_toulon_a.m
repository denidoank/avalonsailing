...
function generate_target_circles_toulon_a
% function generate_target_circles_toulon_a.m
% For a trip from Toulon to the sea and back.
% Script to make const table for the route of the boat,
% in C++.
% Produces a file named "target_circle_points_table_toulon_a.h" in the current directory.
...
[fid, msg] = fopen("target_circle_points_table_toulon_a.h", "w");
if fid == -1
  msg
  error(msg);
  return;
...
% 50 miles leg 1
leg_1 = 50 * 1852;
delta_lat1 = leg_1 * 0.707 * meter2degree;
delta_lon1 = leg_1 * 0.707 * meter2degree * 1 / cos(42.0 / 180 * pi);
delta_leg_1 = [-delta_lat1 -delta_lon1 0];

% 40 miles leg 2
leg_2 = 40 * 1852;
% rotation by using something else than [0.707, 0.707]
delta_lat2 = leg_2 * 0.707 * meter2degree;
delta_lon2 = leg_2 * 0.707 * meter2degree * 1 / cos(42.0 / 180 * pi);
