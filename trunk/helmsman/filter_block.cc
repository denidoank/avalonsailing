...
// The IMU measurements are outputs of a Kalman-Bucy-filtend as such
...
// fluctuations of the measured wind overlayd with the errors
...
// 2. wind sensor fault -> no update (not recognized)
// 3. IMU fault -> no speed nor direction -> no true wind
...
void FilterBlock::Filter(const ControllerInput& in,
    ...
    fil->angle_aoa = SymmetricRad(wrap_long_av_aoa_.Filter(SymmetricRad(alpha_wind_rad + kWindSensorOffsetRad)));
