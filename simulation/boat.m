...
%                         | x_axis_S
%   x_____________________|_____________           x : CAD reference point used in Endbericht, Anhang A.13
%   |              X Deck             /
% ~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ /~~~~~~~
%   |              X Hull           /
%   |                              /
...
% for rudder, keel, hull, sail, center of gravity, bomb, deck, wind sensor
...
B.K = [1.8+coa_K(1); 0; -0.7-coa_K(3)] - cog; % point at keel axis, approx above the middle height of the keel
...
B.R = [0.36; 0; -0.45-coa_R(3)] - cog; % point over mast axis, in middle height of sail
...
B.S = [2.25; 0; deck_to_leech+coa_S(3)] - cog;        % point *over* (thatswhy +) mast axis, in middle height of sail

% Keel Bomb
B.length_B = 1.0;    % m
B.diameter_B = 0.25; % m 
B.c_drag_B = 0.019;  % undocumented, extrapolated from endbericht p. 86 for diameter to length = 25%
B.c_drag_z_B = 0.04;  %  a guess
B.B =  [1.8+0.307/2; 0; -0.7-1.5-0.25/2] - cog;  % keel root, keel length, half bomb diameter


% drive features
% 
% interface: angle reference value in degree or radians?
...
A_eff_x=0.0222;  % m^2, 0.4  0.3  comment: This would results in a resistance force of 261N which contradicts the endbericht significantly.
...
B.damping_rot_y = c_damp_rot_water(A_eff_x + A_eff_z, B.length_H);  % modeled as a plate of a certain area and length in water  
