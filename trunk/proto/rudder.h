...
#define OFMT_RUDDERPROTO_CTL(x) \
	"rudderctl: timestamp_ms:%lld rudder_l_deg:%.3lf rudder_r_deg:%.3lf sail_deg:%.1lf storm_flag:%d\n" \
	, (x).timestamp_ms, (x).rudder_l_deg, (x).rudder_r_deg, (x).sail_deg, (x).storm_flag
...
#define IFMT_RUDDERPROTO_CTL(x, n) \
	"rudderctl: timestamp_ms:%lld rudder_l_deg:%lf rudder_r_deg:%lf sail_deg:%lf storm_flag:%d\n%n" \
