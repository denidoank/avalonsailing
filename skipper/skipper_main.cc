...
int snprint_skipper_out(char *line, int size, double alpha_star_deg) {
    return snprintf(line, size,
                  "timestamp_ms:%lld alpha_star_deg:%f \n",
...
int main(int argc, char* argv[]) {
  ...
  // Need SkipperInput from helmsman, Ais from ais, send to helmsman
...
/*
  ...
  for (;;) {
    ...
    if (skipper_out_pending) set_fd(&wfds, &max_fd, rudd);
    ...
    // change This to skipper_input
    if (FD_ISSET(fileno(rudd), &rfds)) {
      char line[1024];
      while (fgets(line, sizeof line, rudd)) {
       // n = sscan_skipper_input(line, &skipper_input)
    ...
    if (true) {    // send desired angle to helmsman
      char skipper_out_text[1024];
      int n = snprint_skipper_out(skipper_out_text, sizeof rudcmd, ctrl_out.drives_reference);
      if (n <= 0 || n > sizeof skipper_out_text) crash("printing skipper_out_text");
      if (fputs(skipper_out_text, rudd) == EOF) crash("Could not send skipper_out_text");  // TODO allow for a couple of EAGAINS
      skipper_out_pending = true;
      if (forward) Client::Puts(skipper_out_text);       // nice for debugging
    }
