...
int main(int argc, char* argv[]) {
	...
	while (!feof(stdin)) {
		...
		if (!isnan(rudd.sail_deg) && !isnan(wind.angle_deg)) {
			...
			double f_x = f_sail * sin(rad(rudd.sail_deg)) - sqr(imu.vel_x_m_s) * kDrag_x;
			double f_y = f_sail * cos(rad(rudd.sail_deg)) - sqr(imu.vel_y_m_s) * kDrag_y;

			const double kMBoat = 120.0;   // mass of boat
			...
			if (rudder_deg == 0.0) {
				...
				imu.gyr_z_rad_s = speed_m_s / (kRudderArm_m * tan(M_PI-rad(rudder_deg)));
